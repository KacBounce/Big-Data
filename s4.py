...
def genetic_algorithm(products, product_to_station, reconfig_times, prod_times, demands, start_date, generations=100, population_size=100, elite_size=10):
...
def evaluate_schedule(schedule, product_to_station, reconfig_times, prod_times):
    ...
    for task in schedule:
        ...
        # Try to find an available machine that can accommodate the task
        machine = None
        earliest_start_time = None

        # Check all machines for availability
        for possible_machine in product_to_station[product]:
            # Get the last end time for the machine (or assume it starts at 0)
            last_machine_end_time = last_end_time.get(
                possible_machine, task['start_time'])

            # Choose the earliest possible machine to start without a gap
            if earliest_start_time is None or last_machine_end_time < earliest_start_time:
                machine = possible_machine
                earliest_start_time = last_machine_end_time

        # Update the task's start time to minimize idle time
        task['start_time'] = earliest_start_time

        # Update the last end time for the selected machine
        end_time = task['start_time'] + timedelta(minutes=production_time)
        last_end_time[machine] = end_time

        # Add reconfiguration time if applicable (between consecutive tasks on the same machine)
        if machine in last_end_time and len(schedule) > 1:
            prev_task = schedule[schedule.index(task) - 1]
            prev_product = prev_task['product']
            reconfig_time = reconfig_times[prev_product].get(product, 0)
